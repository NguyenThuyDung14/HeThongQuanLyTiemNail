<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Album_Mau_Nail.aspx.cs" Inherits="Album_Mau_Nail" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title></title>
    <script src="JS/footer.js"></script>
    <script src="JS/owl.carousel.min.js"></script>
    <script src="JS/jquery-3.3.1.min.js"></script>
    <link href="CSS/bootstrap.min.css" rel="stylesheet" />
    <link href="CSS/font-awesome.min.css" rel="stylesheet" />
    <link href="CSS/footer.css" rel="stylesheet" />
    <link href="CSS/jquery.fancybox.min.css" rel="stylesheet" />
    <link href="CSS/style.css" rel="stylesheet" />
    <link href="CSS/bootstrap.min.css" rel="stylesheet" />
    <link href="CSS/owl.theme.default.min.css" rel="stylesheet" />
    <link href="CSS/AlbumMauNail.css" rel="stylesheet" />
</head>
<body>
    <form id="form1" runat="server">
        <div class="tieude"><span>ALBUM MẪU NAILS</span></div>
        <div class="base">
            <div class="container">
                <div class="row">
                    <div class="block-nailbox col-sm-5">
                        <img src="image/MauNail/Nail_Tay/1.png" />
                        <p>Mẫu Nail Trendy</p>
                    </div>
                    <div class="block-nailbox col-sm-5">
                        <img src="image/MauNail/Nail_Tay/Nail_Co_Dau_1.png" />
                        <p>Mẫu Nail Cô Dâu</p>
                    </div>
                    <div class="block-nailbox col-sm-5">
                        <img src="image/MauNail/Nail_Tay/Nail_Den_1.png" />
                        <p>Mẫu Nail Màu Đen</p>
                    </div>
                    <div class="block-nailbox col-sm-5">
                        <img src="image/MauNail/Nail_Tay/Nail_Giang_Sinh.png" />
                        <p>Mẫu Nail Giáng Sinh</p>
                    </div>
                    <div class="block-nailbox col-sm-5">
                        <img src="image/MauNail/Nail_Tay/Nail_Ombre_1.png" />
                        <p>Mẫu Nail Ombre</p>
                    </div>
                    <div class="block-nailbox col-sm-5">
                        <img src="image/MauNail/Nail_Tay/Nail_Tet_1.png" />
                        <p>Mẫu Nail Đón Tết</p>
                    </div>
                </div>

            </div>
        </div>

    </form>
</body>
<footer>
    <div class="footer-phone">
        <div class="tab-nav-container">
            <a href="Default.aspx" class="tab">
                <img src="image/Footer/sala.png" style="width: 221px; margin-top: -78px;" />
                <p>Trang chủ</p>
            </a>
            <a href="tell:0789448907">
                <div class="tab purple">
                    <img src="image/Footer/phone.png" />
                    <p>Gọi điện</p>
                </div>
            </a>
            <a href="#">
                <div class="tab pink">
                    <img src="image/Footer/messenger.png" />
                    <p>Nhắn tin</p>
                </div>
            </a>
            <a href="/thong-tin-khach-hang">
                <div class="tab teal">
                    <img src="image/Footer/admin%20.png" />
                    <p>Thông tin</p>
                </div>
            </a>
        </div>
    </div>
</footer>
</html>
